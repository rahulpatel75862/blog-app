...
export const getComments = async(req, res, next) => {
  if(!req.user.isAdmin){
    return next(errorHandler(403, 'You are not alloed to get all comments!'))
  }
  try{
    ...
    return res.status(200).json({comments, totalComments, lastMonthsComments})
  } catch (error) {
    next(error);
  }
}
...
export const editComment = async(req, res, next) => {
  try{
    ...
    if(comment.userId === req.user.id && !res.user.isAdmin){
      return next(errorHandler(404, 'You are not allowed to edit that comment!'))
    }
